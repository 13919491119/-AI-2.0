...
if [ -f .env.wechat ]; then
  ...
  export $(grep -E '^(WECHAT_ENCODING_AES_KEY|WECHAT_APPID|WECHAT_TOKEN|WECHAT_ACCEPT_TOKENS|WECHAT_REDIS_URL|STRICT_WECHAT_VERIFY|WECHAT_PORT|USE_NGROK)=' .env.wechat | xargs)
