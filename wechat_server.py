...
def handle_command(command):
    ...
    try:
        # 联网检索指令：以“联网查询/搜索/web:/research:”或“url:/抓取:”开头
        lower_cmd = command.strip().lower()
        if lower_cmd.startswith(('联网查询', '搜索', 'web:', 'research:', 'url:', '抓取')):
            try:
                from internet_research import research_and_summarize  # type: ignore
                result = research_and_summarize(command, max_results=3)
                summary = result.get('summary', '')
                sources = result.get('sources', [])
                cites = "\n".join([f"- {u}" for u in sources]) if sources else "- (无引用或未启用搜索API)"
                return f"【联网检索摘要】\n{summary}\n\n参考：\n{cites}"
            except Exception as ie:
                logger.error(f"联网检索失败: {ie}")
                # 不中断主链路，继续尝试后端API
        last_err = None
