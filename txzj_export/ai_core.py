...
class CelestialNexusAI:
    ...
    async def _execute_self_upgrade(self):
        ...
        # 支持三种升级模式：auto, dry-run, manual
        # - auto: 自动执行（风险高，仅在受控环境开启）
        # - dry-run: 生成升级计划并写入等待批准文件，但不执行任何步骤
        # - manual: 与 dry-run 类似，标记为需人工审核
        import os, json
        upgrade_plan = await self._generate_upgrade_plan()
        mode = os.getenv('XUANJI_UPGRADE_MODE', os.getenv('XUANJI_AUTO_UPGRADE', 'dry-run'))
        mode = mode.lower()
        pending_path = 'upgrade_plan_pending.json'
        if mode not in ('auto', 'dry-run', 'manual'):
            mode = 'dry-run'

        if mode in ('dry-run', 'manual'):
            # 仅生成计划并持久化，等待人工审批
            try:
                ...
                # 标记在 upgrade_plans 调度队列中以便可视化/人工接管
                self.upgrade_plans.setdefault('scheduled_upgrades', []).append({
                    'plan_summary': upgrade_plan.get('expected_improvements', {}),
                    'generated_at': datetime.now().isoformat(),
                    'mode': mode,
                    'status': 'pending_approval'
                })
                self.logger.info(f"升级计划已生成 (mode={mode}) 并写入 {pending_path}，等待人工审批")
            except Exception as e:
                self.logger.error(f"写入升级计划失败: {e}")
            return

        # 若为 auto，则按原有流程执行（高风险，请仅在信任环境下启用）
        try:
            ...
            self.upgrade_plans.setdefault("completed_upgrades", []).append(upgrade_record)
            self.learning_memory.setdefault("adaptation_history", []).append(upgrade_record)
            self.logger.info("自主升级完成")
        except Exception as e:
            self.logger.error(f"自动升级流程异常: {e}")
            # 将失败信息写入待审计划以便人工介入
            try:
                with open(pending_path, 'w', encoding='utf-8') as f:
                    json.dump({'plan': upgrade_plan, 'error': str(e), 'generated_at': datetime.now().isoformat(), 'status': 'failed_auto'}, f, ensure_ascii=False, indent=2)
            except Exception:
                pass
